...
const Header = () => {
  ...
  return (
    <header className={isScroll?'bg-[#0A0A37] sticky top-0 z-30':'bg-[#0A0A37]'} >
        <div className='h-28 flex justify-between items-center w-[92%] mx-auto'>
            <div>
                <a className='cursor-pointer' href={'/'}>
                    <img src={'/assets/img/dsla_logos/DSLA_Logo_Portal_&_Loading.svg'} alt={''} width={250} height={29}/>
                </a>
            </div>
            ...
            <button className='toggle cursor-pointer lg:hidden' onClick={toggleMenu}>
                {isClick ?(<IoMenu color='white' size={50}/>):(<IoCloseSharp color='white' size={50}/>)}    
            </button>
        </div>    
    </header>
  )
